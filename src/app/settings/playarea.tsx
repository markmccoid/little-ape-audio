...
const playarea = () => {
  ...
  const handleScroll = useAnimatedScrollHandler({
    ...
    onEndDrag: (e) => {
      ...
      flSharedIndex.value = e.targetContentOffset.x / width;
    },
  });
  ...
  const titleValue = useDerivedValue(() => {
    return interpolate(scrollX.value / width, [0, 1], [1, 0], {});
  });

  const titleAnim = useAnimatedStyle(() => {
    const op = interpolate(scrollX.value / width, [0, 1], [1, 0], {});
    return {
      opacity: op,
    };
  });
  ...
  const textAnimStyle = useAnimatedStyle(() => {
    const dotPos = interpolate(scrollX.value / width, [0, 1], [width / 2, 0], {
      // const dotPos = interpolate(scrollX.value, [0, width], [-78, -33], {
      // extrapolateRight: Extrapolation.CLAMP,
    });
    // console.log("DOT", dotPos, scrollX.value / width);
